@if (Cart::instance('cart')->count() > 0 && Cart::instance('cart')->products()->count() > 0)
    ...
    <div class="cart-action cart-action d-flex gap-2">
        <a class="btn btn-brand-1-full hover-up" href="{{ route('public.cart') }}">{{ __('View Cart') }}</a>
        <a class="btn btn-brand-1-full hover-up" href="{{ route('public.checkout.information', OrderHelper::getOrderSessionToken()) }}">{{ __('Checkout') }}</a>
    </div>
@endif
